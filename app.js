(function(){
  ...
  // QR library loads on-demand (only when a teacher views the dashboard) so
  // student pages — the vast majority of traffic — never fetch/parse it at all.
  ...
  function renderTeacherResultsInner(testId, loadError){
    ...
    const scoredCount = teacherResultsCache.filter(r=>r.scored).length;
    const rows = visible.map(r=>`
      <tr class="${r.violations>0?'flagged':''}">
        ...
        <td class="mono">${r.scored ? `${r.score}/${r.total}` : '<span class="status-pill skip">Not scored</span>'}</td>
        <td>${r.scored ? r.correct : '—'}</td>
        <td>${r.scored ? r.wrong : '—'}</td>
        <td>${r.scored ? r.unattempted : '—'}</td>
    ...
    app.innerHTML = `
      ...
      <div class="panel" style="margin-bottom:16px; padding:14px 18px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
        <div>
          <div style="font-weight:600; margin-bottom:2px;">${scoredCount} of ${teacherResultsCache.length} attempts scored</div>
          <div class="helper">Make sure correct answers are set on the test, then generate — safe to click again after any fix, it re-scores everyone.</div>
        </div>
        <button class="btn" id="generate-results-btn" onclick="app_generateResults()">Generate results</button>
      </div>
  ...
  window.app_generateResults = async function(){
    const btn = document.getElementById('generate-results-btn');
    if(btn){ btn.disabled = true; btn.innerHTML = '<span class="btn-spinner"></span>Generating…'; }
    try{
      await apiCall('generateResults', {testId: currentResultsTestId}, 30000);
    }catch(e){ showError(e); }
    await window.app_viewResults(currentResultsTestId, encodeURIComponent(teacherResultsTestTitle));
  };
  ...
  window.app_downloadResultsCsv = function(){
    ...
    const rows = results.map(r=>[
      ...
      r.scored ? r.score : 'Not scored', r.scored ? r.total : '', r.scored ? r.correct : '', r.scored ? r.wrong : '', r.scored ? r.unattempted : '',
  ...
  function buildPrintableReportHtml(title, results){
    const summaryRows = results.map(r=>`
      <tr><td>${escapeHtml(r.studentName)}</td><td>${escapeHtml(r.rollNo||'—')}</td><td>${r.scored ? r.score+'/'+r.total : 'Not scored'}</td>
      <td>${r.scored ? r.correct : '—'}</td><td>${r.scored ? r.wrong : '—'}</td><td>${r.scored ? r.unattempted : '—'}</td><td>${fmtTime(r.timeTakenSec)}</td>
  ...
  window.app_addQ = function(){
    // Defaults are pre-filled with placeholders (a blank-looking space, and
  ...
  window.app_saveTest = async function(){
    ...
    // Text/options/correct-answer are intentionally NOT required — scoring
    // happens later via "Generate results", so you can fill these in anytime
    // before then (even after students have already attempted the test).
  ...
  function scheduleSaveProgress(immediate){
    ...
    if(immediate) doSave();
    else progressSaveTimer = setTimeout(doSave, 2000);
  }
